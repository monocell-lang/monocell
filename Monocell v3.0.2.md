...
**# Static variables**  
**# Const**  
**# Dynamic allocation, copy and move**  
...
```
```
   double PI = 3.141_159_653_589_793_238_463;
...
Example: bubble sort  
```
int[_] BubbleSort(int mut[_] arr)
{
   if (arr[::] < 2)
      return arr;
...
### Nesting optionals (made sure its still readable 🙏🤲)  
...
```
```
void pet_animal(home::Animal animal);  //❌ private type
...
🌿 No reading from terminal or writing to files. Writing to terminal reading from files is OK tho)  
...
```
```
Deallocation:   regret[size, alignment] pointer   ->  void
...
```
```
   void @swap(IntVec mut& left, IntVec mut& right)
...
```
      ...
      IntVec mut buffer = noinit;

```
```
      std::mem_copy(&buffer, &left, @sizeof(IntVec));
      std::mem_copy(&left, &right, @sizeof(IntVec));
      std::mem_copy(&right, &buffer, @sizeof(IntVec));
...
```
```
   IntVec[2] arr = { init({ 1, 2 }), init({ 3, 4 });
   @swap(arr[0], arr[1]);
...
// NOTE: void @swap() _; ALWAYS exists

```
  
🤝 Table of compiler handshakes  
```
|------------------|---------------|---------------------|

```
```
| system function  | normal call   | user dont implement |
...
```
```
| @clone           | a.clone()     | ❌ not exist        |
| @destruct        | a.destruct()  | ❌ not exist        |
| @swap            | swap(a, b)    | ✅ always exists    |
...
Example 6: another trap (this one 💣🧨)  
```
struct IntVec {
...
* The terms are kinda brain twisting 🧠🌀 (actually very brain twisting 💀) but they reflect real processes  
    ...
    - Swap: exchanging resource ownership  
...
* 👉 That means one has to give up the resource  
* You can not overload move. Move = shallow bitwise copy + setting the variable unusable  
...
Example 2: move is the same everywhere  
```
void testA()
...
```
```
   Controler con = Controler { true, { 190, 10, 20 }, true };
   take_control(mov con);
...
```
```
   // ❌ arr[0] and arr[1] are references. You can not move a variable thru T&😬
...
Example 4: easy but tell u anyway 🫲🙂‍↔️🫱  
```
int x = mov 3;        //🫷😖 3 is Rvalue, cant move
int y = mov (x + 6);  //😬 same for x + 6

```
  
Example 5: mystery solved🤩  
```
struct CopyMe {}
...
```
   ...
   // Answer: ❌ nah, var is forced move! (its ded anyway)
}
...
```
```
   // Answer: ✅ yep, cant move through ref. 👉 must copy
